	...
	module.exports = new Class({
		...
		, init: function() {
            ...
            this._modelListenerNames.filter(function(config) {
                if (type.function(this[config.listenerName])) this._modelListners.push({event: config.eventName, listner: this[config.listenerName]});
            }.bind(this));

            this._resourceListenerNames.filter(function(config) {
                if (type.function(this[config.listenerName])) this._resourceListners.push({event: config.eventName, listner: this[config.listenerName]});
            }.bind(this));
        }
